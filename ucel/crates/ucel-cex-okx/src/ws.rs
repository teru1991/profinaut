...
impl OkxWsAdapter {
    pub fn spot() -> Self {
        Self {
            exchange_id: "okx-spot",
            inst_type: "SPOT",
        }
    }
    pub fn swap() -> Self {
        Self {
            exchange_id: "okx-swap",
            inst_type: "SWAP",
        }
    }
    pub fn futures() -> Self {
        Self {
            exchange_id: "okx-futures",
            inst_type: "FUTURES",
        }
    }
    pub fn option() -> Self {
        Self {
            exchange_id: "okx-option",
            inst_type: "OPTION",
        }
    }
}
...
impl WsVenueAdapter for OkxWsAdapter {
    ...
    async fn fetch_symbols(&self) -> Result<Vec<String>, String> {
        fetch_symbols_by_inst_type(self.inst_type).await
    }
    ...
    fn build_subscribe(
        ...
        let channel = op_id
            .strip_prefix("okx.public.ws.")
            .ok_or("bad family_id")?;
        if symbol.is_empty() {
            let msg = json!({"op":"subscribe","args":[{"channel":channel}]});
            return Ok(vec![OutboundMsg {
                text: msg.to_string(),
            }]);
        }
    ...
    fn classify_inbound(&self, raw: &[u8]) -> InboundClass {
        ...
        InboundClass::Data {
            op_id: Some(format!("okx.public.ws.{chan}")),
            symbol: None,
