...
use ucel_subscription_planner::{generate_plan_v2, load_coverage_v2};
...
fn gmocoin_public_coverage_v2_all_families_build_subscribe_ok() {
    ...
    for k in plan.seed {
        ...
        assert!(
            msgs.is_ok(),
            "build_subscribe failed: key={:?} err={:?}",
            k,
            msgs.err()
        );
    }
...
fn gmocoin_private_coverage_v2_all_families_build_subscribe_ok() {
    ...
    let dummy = GmoCredentials {
        api_key: "DUMMY".into(),
        api_secret: "DUMMY".into(),
    };
