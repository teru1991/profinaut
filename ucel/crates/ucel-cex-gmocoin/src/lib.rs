...
use tokio::sync::mpsc;
use ucel_core::{
    ErrorCode, Exchange, OpName, OrderBookDelta, OrderBookLevel, OrderBookSnapshot, TradeEvent,
    UcelError,
};
use ucel_transport::{
...
const WS_CHANNELS: [WsChannelSpec; 12] = [
    ...
    WsChannelSpec {
        id: "crypto.public.ws.trades.update",
        ws_url: "wss://api.coin.z.com/ws/public/v1",
        channel: "trades",
        requires_auth: false,
    },
    WsChannelSpec {
        id: "crypto.public.ws.orderbooks.update",
        ws_url: "wss://api.coin.z.com/ws/public/v1",
        channel: "orderbooks",
        requires_auth: false,
    },
    WsChannelSpec {
        id: "crypto.private.ws.executionevents.update",
        ws_url: "wss://api.coin.z.com/ws/private/v1/{token}",
        channel: "executionEvents",
        requires_auth: true,
    },
    WsChannelSpec {
        id: "crypto.private.ws.orderevents.update",
        ws_url: "wss://api.coin.z.com/ws/private/v1/{token}",
        channel: "orderEvents",
        requires_auth: true,
    },
    WsChannelSpec {
        id: "crypto.private.ws.positionevents.update",
        ws_url: "wss://api.coin.z.com/ws/private/v1/{token}",
        channel: "positionEvents",
        requires_auth: true,
    },
    WsChannelSpec {
        id: "fx.public.ws.ticker.update",
        ws_url: "wss://api.coin.z.com/fx/ws/public/v1",
        channel: "ticker",
        requires_auth: false,
    },
    WsChannelSpec {
        id: "fx.public.ws.trades.update",
        ws_url: "wss://api.coin.z.com/fx/ws/public/v1",
        channel: "trades",
        requires_auth: false,
    },
    WsChannelSpec {
        id: "fx.public.ws.orderbooks.update",
        ws_url: "wss://api.coin.z.com/fx/ws/public/v1",
        channel: "orderbooks",
        requires_auth: false,
    },
    WsChannelSpec {
        id: "fx.private.ws.executionevents.update",
        ws_url: "wss://api.coin.z.com/fx/ws/private/v1/{token}",
        channel: "executionEvents",
        requires_auth: true,
    },
    WsChannelSpec {
        id: "fx.private.ws.orderevents.update",
        ws_url: "wss://api.coin.z.com/fx/ws/private/v1/{token}",
        channel: "orderEvents",
        requires_auth: true,
    },
    WsChannelSpec {
        id: "fx.private.ws.positionevents.update",
        ws_url: "wss://api.coin.z.com/fx/ws/private/v1/{token}",
        channel: "positionEvents",
        requires_auth: true,
    },
...
#[derive(Debug, Clone, Deserialize)]
struct BookWsMsg {
    asks: Vec<BookLevelWs>,
    bids: Vec<BookLevelWs>,
    sequence: Option<u64>,
}
#[derive(Debug, Clone, Deserialize)]
struct BookLevelWs {
        ...
        if let Some(next) = self.next_sequence {
            if delta.sequence_start != next {
                ...
                return Err(UcelError::new(
                    ErrorCode::Desync,
                    "orderbook gap detected; resync required",
                ));
            }
    ...
    pub fn apply_snapshot(
        ...
        while let Some(delta) = self.buffered_deltas.pop_front() {
            ...
            for delta_bid in delta.bids {
                if let Some(existing_bid) = snapshot
                    .bids
                    .iter_mut()
                    .find(|b| b.price == delta_bid.price)
                {
            ...
            for delta_ask in delta.asks {
                if let Some(existing_ask) = snapshot
                    .asks
                    .iter_mut()
                    .find(|a| a.price == delta_ask.price)
                {
        ...
        match channel_id {
            ...
            "crypto.public.ws.trades.update" | "fx.public.ws.trades.update" => {
                ...
                Ok(MarketEvent::Trade(TradeEvent {
                    trade_id: format!("{}:{}", m.symbol, m.price),
                    price: parse_num(&m.price)?,
                    qty: parse_num(&m.size)?,
                    side: m.side,
                }))
            }
                ...
                Ok(MarketEvent::OrderBookSnapshot(OrderBookSnapshot {
                    bids: m
                        ...
                        .map(|l| OrderBookLevel {
                            price: parse_num(&l.price).unwrap_or(0.0),
                            qty: parse_num(&l.size).unwrap_or(0.0),
                        })
                        .collect(),
                    asks: m
                        .asks
            ...
            _ => Err(UcelError::new(
                ErrorCode::NotSupported,
                format!("unknown ws channel: {channel_id}"),
            )),
        }
    }
}
...
fn ws_spec(id: &str) -> Result<&'static WsChannelSpec, UcelError> {
    WS_CHANNELS
        .iter()
        .find(|c| c.id == id)
        .ok_or_else(|| UcelError::new(ErrorCode::NotSupported, format!("unknown ws channel: {id}")))
}
...
fn parse_num(s: &str) -> Result<f64, UcelError> {
...
mod tests {
    ...
    async fn ws_contract_all_channels_have_typed_parse_and_subscribe() {
        ...
        for spec in GmoCoinWsAdapter::channel_specs() {
            let symbol = if spec.channel == "ticker"
                || spec.channel == "trades"
                || spec.channel == "orderbooks"
            {
                Some("BTC_JPY".to_string())
            } else {
                None
            };
    ...
    async fn reconnect_and_resubscribe_increments_metrics_and_is_idempotent() {
        ...
        let sub = WsSubscription {
            channel_id: "crypto.public.ws.ticker.update",
            symbol: Some("BTC_JPY".into()),
        };
        ws.connect_and_subscribe(&transport, sub.clone(), None)
            .await
            .unwrap();
        ws.connect_and_subscribe(&transport, sub, None)
            .await
            .unwrap();
        ws.reconnect_and_resubscribe(&transport, None)
            .await
            .unwrap();
    ...
    async fn private_ws_preflight_rejects_without_auth_before_connect() {
        ...
        let err = ws
            .connect_and_subscribe(
                &transport,
                WsSubscription {
                    channel_id: "crypto.private.ws.orderevents.update",
                    symbol: None,
                },
                None,
            )
    ...
    fn strict_coverage_gate_requires_zero_gaps() {
        let manifest_path =
            std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("../../coverage/gmocoin.yaml");
