use super::access::{ensure_attachment_size, JpPolitenessPolicy};
...
fn rows() -> Result<Vec<FixtureArtifact>, JpOfficialError> {
...
fn bytes_map() -> Result<serde_json::Map<String, serde_json::Value>, JpOfficialError> {
...
fn to_descriptor(a: &FixtureArtifact) -> Result<IrArtifactDescriptor, JpOfficialError> {
    Ok(IrArtifactDescriptor {
        ...
        kind: parse_kind(&a.kind).ok_or_else(|| {
            JpOfficialError::new(JpOfficialErrorCode::UnsupportedArtifactKind, &a.kind)
        })?,
...
pub fn list(request: &IrArtifactListRequest) -> Result<IrArtifactListResponse, JpOfficialError> {
    let artifacts = rows()?
        .into_iter()
        .filter(|x| {
...
pub fn fetch(
...
) -> Result<IrArtifactFetchResponse, JpOfficialError> {
    let row = rows()?
        .into_iter()
        .find(|x| {
        ...
        .ok_or_else(|| {
            JpOfficialError::new(JpOfficialErrorCode::ArtifactNotFound, "artifact not found")
        })?;
    if let Some(ct) = &row.content_type {
            ...
            || ct.contains("text"))
        {
            return Err(JpOfficialError::new(
                JpOfficialErrorCode::InvalidContentType,
                ct.clone(),
            ));
        }
    }
    ...
    Ok(IrArtifactFetchResponse {
        ...
        source_metadata: serde_json::json!({"source_url": row.source_url, "size_bytes": row.size_bytes}),
    })
}
