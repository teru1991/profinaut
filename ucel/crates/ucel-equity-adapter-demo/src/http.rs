...
impl DemoHttpClient {
    pub fn get_json(&self, endpoint: &str) -> Result<serde_json::Value, EquityAdapterError> {
        match endpoint {
            "/quote/7203.T" => Ok(serde_json::json!({"symbol":"7203.T","bid":3500.0,"ask":3501.0,"last":3500.5,"ts_ms":1700000000000_u64,"latency":"delayed"})),
            ...
            "/calendar/JP/2026-03-06" => Ok(serde_json::json!({"market":"JP","exchange":"TSE","timezone":"Asia/Tokyo","date":"2026-03-06","sessions":[{"kind":"Regular","start":"09:00","end":"15:00"}]})),
            "/actions/7203.T" => Ok(serde_json::json!([
                {"type":"dividend","ex_date":"2026-03-28","amount":45.0,"currency":"JPY"}
            ])),
            _ => Err(EquityAdapterError::new(EquityAdapterErrorKind::UnsupportedSymbol, "demo endpoint not found")),
        }
    }
}
