...
pub fn get_bars(
    ...
    validate_bar_timeframe(timeframe).map_err(|e| {
        EquityAdapterError::new(EquityAdapterErrorKind::MalformedResponse, e.message)
    })?;
    ...
    let arr = raw.as_array().ok_or_else(|| {
        EquityAdapterError::new(EquityAdapterErrorKind::MalformedResponse, "bars not array")
    })?;
    let mut out = Vec::new();
    for it in arr.iter().take(limit) {
        let latency = match it
            .get("latency")
            .and_then(|v| v.as_str())
            .unwrap_or("delayed")
        {
