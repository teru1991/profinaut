...
fn required_keys_reject_empty() {
    let err = ObsRequiredKeys::try_new("binance", "c1", "book", "", "run1", "trace1", "req1").unwrap_err();
    assert!(err.message.contains("required keys"));
}

#[test]
fn required_keys_span_constructs() {
    let k = ObsRequiredKeys::try_new("binance", "c1", "book", "BTCUSDT", "run1", "trace1", "req1").unwrap();
