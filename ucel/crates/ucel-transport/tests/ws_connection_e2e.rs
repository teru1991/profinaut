                ...
                accept = listener.accept() => {
                    ...
                    tokio::spawn(async move {
                        ...
                        if let Some(Ok(Message::Text(t))) = r.next().await {
                            ...
                            if first_connection_close && is_first {
                                let _ = w.send(Message::Close(None)).await;
                                return;
                            }
...
async fn e2e_reconnect_drip_wal() {
    ...
    let run = tokio::spawn(async move { run_ws_connection(adapter, rules, &mut store, wal, cfg, shutdown).await });
