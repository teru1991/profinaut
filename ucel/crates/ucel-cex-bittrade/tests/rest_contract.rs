...
impl Transport for SpyTransport {
    async fn send_http(
        ...
        self.queue.lock().unwrap().pop_front().unwrap_or_else(|| {
            Ok(HttpResponse {
                status: 500,
                body: Bytes::from_static(br#"{"code":"500"}"#),
            })
        })
    }
...
async fn private_preflight_rejects_without_transport_call() {
    ...
    let err = client
        .execute(
            "private.rest.order.get",
            RequestArgs {
                path_params: BTreeMap::from([("order-id".into(), "1".into())]),
                ..Default::default()
            },
            None,
        )
...
async fn public_context_never_uses_key_path() {
    ...
    transport.enqueue_ok(
        200,
        Bytes::from_static(br#"{"status":"ok","data":["BTC"]}"#),
    );
...
async fn maps_429_with_retry_after_and_stops() {
    ...
    transport.enqueue_ok(
        429,
        Bytes::from_static(br#"{"code":"429","message":"limited","retry_after_ms":123}"#),
    );

    let err = client
        .execute(
            "public.rest.common.symbols.get",
            RequestArgs::default(),
            None,
        )
        .await
...
fn strict_coverage_gate_for_bittrade_is_zero_gap() {
    let manifest_path =
        std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("../../coverage/bittrade.yaml");
