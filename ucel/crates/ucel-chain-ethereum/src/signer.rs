...
pub trait EvmSigner: Send + Sync {
    fn sign_transaction(&self, tx: &EvmTransactionRequest) -> Result<EvmSignedTransaction, UcelError>;
}
...
impl EvmSigner for DeterministicTestSigner {
    fn sign_transaction(&self, tx: &EvmTransactionRequest) -> Result<EvmSignedTransaction, UcelError> {
        if self.signer_id.is_empty() {
            return Err(reason_to_error(EvmReasonCode::ExecutionReverted, "signer_id empty"));
        }
        let body = format!(
            "{}:{}:{}:{}",
        ...
        Ok(EvmSignedTransaction { tx_hash: hash.clone(), raw_tx_hex: format!("0xraw{}", hash.trim_start_matches("0x")) })
    }
}

pub fn redact_signer_material(input: &str) -> String {
    input.replace("private", "[redacted]").replace("secret", "[redacted]")
}
