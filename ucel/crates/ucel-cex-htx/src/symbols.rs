...
fn d_f64(v: f64) -> Result<Decimal, String> {
    // f64->string 経由（HTX payload は小数で返ることがある）
...
pub async fn fetch_symbol_snapshot() -> Result<Snapshot, String> {
    ...
    if body.status != "ok" && body.status != "OK" {
        return Err(format!("htx symbols api status={}", body.status));
    }
    ...
    if instruments.is_empty() {
        return Err("htx: no instruments produced".into());
    }
...
pub async fn fetch_market_meta() -> Result<BTreeMap<String, MarketMeta>, String> {
    ...
    for s in snapshot.instruments {
        let mut mm = MarketMeta::new(
            MarketMetaId::new(Exchange::Htx, MarketType::Spot, s.raw_symbol.clone()),
            s.tick_size,
            s.lot_size,
        );
        mm.base = Some(s.base);
        ...
        mm.validate_basic()
            .map_err(|e| format!("htx invalid meta symbol={} err={e}", s.raw_symbol))?;
...
mod tests {
    ...
    #[test]
    fn htx_maps_precision_to_tick_step_and_min() {
    ...
    #[test]
    fn htx_zero_precision_is_allowed_but_tick_step_must_be_positive() {
        let s: Sym = serde_json::from_str(
            r#"{
              "symbol":"btcusdt",
              "state":"online",
