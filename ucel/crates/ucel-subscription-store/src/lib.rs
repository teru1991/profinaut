...
mod tests {
    ...
    #[test]
    fn reopen_file_db_preserves_state() {
        let dir = tempfile::tempdir().unwrap();
        let db = dir.path().join("store.sqlite3");
        {
            let mut store = SubscriptionStore::open(db.to_str().unwrap()).unwrap();
            let row = SubscriptionRow {
                key: "k2".into(),
                exchange_id: "binance".into(),
                op_id: "crypto.public.ws.ticker".into(),
                symbol: Some("BTC/USDT".into()),
                params_json: "{}".into(),
                assigned_conn: Some("c1".into()),
            };
            store.seed(&[row], 1).unwrap();
            store.mark_deadletter("k2", "missing auth", 2).unwrap();
        }
        {
            let store = SubscriptionStore::open(db.to_str().unwrap()).unwrap();
            assert_eq!(store.state_of("k2").unwrap().unwrap(), "deadletter");
        }
    }
}
