...
fn read_catalog_ids() -> BTreeSet<String> {
    ...
    if let Some(arr) = v.get("rpc_http_methods").and_then(|x| x.as_array()) {
        for it in arr {
...
fn coverage_ids_exist_in_catalog_and_are_public_market_data() {
    ...
    let cov = include_str!("../../../coverage/deribit.yaml");
    ...
    for e in entries {
        ...
        assert!(
            id.contains(".public.market_data."),
            "non-H id in deribit H coverage: {id}"
        );
        assert!(
            id.starts_with("jsonrpc.http.public."),
            "non-public id in deribit H coverage: {id}"
        );
        assert_eq!(
