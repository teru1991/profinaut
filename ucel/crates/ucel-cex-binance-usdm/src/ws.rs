...
impl BinanceUsdmWsAdapter { pub fn new() -> Self { Self } }
...
fn stream_template_for_family(op: &str) -> Option<&'static str> {
    match op {
        "binance.usdm.public.ws.aggTrade" => Some("{symbol}@aggTrade"),
        "binance.usdm.public.ws.markPrice" => Some("{symbol}@markPrice@1s"),
...
#[async_trait]
impl WsVenueAdapter for BinanceUsdmWsAdapter {
    fn exchange_id(&self) -> &str { "binance-usdm" }

    fn ws_url(&self) -> String {
        // USD-M Futures WS Market Streams base url is official
