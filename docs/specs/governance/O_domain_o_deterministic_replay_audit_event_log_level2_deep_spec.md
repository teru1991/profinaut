# Domain O — Deterministic Replay / Audit Event Log（Level 2 Deep Spec）
> 条件充足：Non-negotiable（1.1）、Canonical Model/Contract（1.6・1.7）、Behavior/Tests（1.20・1.21）が揃っているため、整理のみでLevel 2を追加出力。 

## 2.1 Non-negotiable（再掲：監査の“守るべき線”）
...
## 2.2 Canonical Model（監査イベントの正準データモデル）
### 2.2.1 Canonical Event Header（正準ヘッダ）
...
### 2.2.2 canonicalization & digest（正規化と要約）
...
## 2.3 Determinism Contract（決定論の契約）
...
## 2.4 Integrity Model（完全性：hash chain / checkpoint / verify）
...
## 2.5 Storage Model（Append-only WAL）
...
## 2.9 Behavior / Tests（CIで担保すべき振る舞い）
...
## 2.10 Capability ↔ Spec Trace（ID保持：要点だけ）
