# GMO Coin Official Docs Catalog (SSOT)

This directory contains an **official-docs-only** catalog for GMO Coin.
...
- websocket.md: WebSocket channels catalog (public/private, crypto/fx)
...
## Verification
```bash
python -c "import json; json.load(open('docs/exchanges/gmocoin/catalog.json')); print('OK JSON parse')"

python - <<'PY'
import json
d=json.load(open('docs/exchanges/gmocoin/catalog.json'))
ids=[]
