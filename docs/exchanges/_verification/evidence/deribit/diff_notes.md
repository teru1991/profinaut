...
- Auth signature/timestamp/header の公式突合が未確定（実装修正前提）。
  - URL: https://docs.deribit.com/api-reference/authentication/public-auth
...
- Rate limit 適用単位（IP/Account/Key）と retry/backoff 指針の明示不足。
  - URL: https://docs.deribit.com/articles/rate-limits
- 数量・価格精度/丸め規則の明示不足。
  - URL: https://docs.deribit.com/api-reference
