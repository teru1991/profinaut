...
|---|---|---|---|---|---|
| OVL-01 | Project progress / delivered scope summary | `README.md`, `docs/plans/roadmap.md`, `docs/changelog.md` | `docs/plans/roadmap.md` for plan + current step state; `docs/changelog.md` for chronological release log; `README.md` only as concise entrypoint | README currently includes long delivered-feature lists that are also represented by roadmap/changelog. Keep README thin, and point to roadmap/changelog for authoritative detail. | 1) Trim README "included" section to short summary + links. 2) Keep roadmap as plan/status. 3) Keep changelog purely date-ordered change history. |
