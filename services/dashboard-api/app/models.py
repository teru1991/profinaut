...
class ExecutionQualityTsRecord(Base):
    __tablename__ = "execution_quality_ts"

    eq_id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    instance_id: Mapped[str] = mapped_column(String(64), ForeignKey("instances.instance_id", ondelete="CASCADE"), nullable=False)
    symbol: Mapped[str] = mapped_column(String(64), nullable=False)
    slippage_bps: Mapped[float] = mapped_column(Numeric(20, 8), nullable=False)
    latency_ms: Mapped[float] = mapped_column(Numeric(20, 8), nullable=False)
    fill_ratio: Mapped[float] = mapped_column(Numeric(20, 8), nullable=False)
    timestamp: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False)
