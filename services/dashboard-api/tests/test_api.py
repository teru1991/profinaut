...
def test_status_summary_success(client, monkeypatch):
    from app.config import get_settings

    get_settings.cache_clear()
    monkeypatch.setenv("MARKETDATA_BASE_URL", "http://marketdata.local")
    monkeypatch.setenv("EXECUTION_BASE_URL", "http://execution.local")
    ...
    assert [component["name"] for component in body["components"]] == [
        "marketdata.healthz",
        "marketdata.capabilities",
        "execution-paper.healthz",
        "execution-paper.capabilities",
    ]
    for component in body["components"]:
        assert component["status"] == "OK"
        assert component["degraded_reason"] is None
        assert component["last_checked_at"]
        assert isinstance(component["latency_ms"], int)


def test_status_summary_partial_failure_returns_down_component(client, monkeypatch):
    import urllib.error
    ...
    def _fake_urlopen(req, timeout):
        assert timeout <= 2.0
        if req.full_url.startswith("http://execution.local/"):
    ...
    assert components["execution-paper.healthz"]["status"] == "DOWN"
    assert components["execution-paper.healthz"]["degraded_reason"]
    assert components["execution-paper.capabilities"]["status"] == "DOWN"
    assert components["execution-paper.capabilities"]["degraded_reason"]
    assert components["marketdata.healthz"]["status"] == "OK"
    assert components["marketdata.capabilities"]["status"] == "OK"
